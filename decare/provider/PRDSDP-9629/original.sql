    select
        ...
        nvl(s.DESCRIPTION,
        gd.DESCRIPTION)                         as SPECIALTY_DESCRIPTION,
        nvl(ps.SPECIALTY_CODE,
        gd.CODE)                            as SPECIALTY_CODE,
        nvl(ps.SPECIALTY_TYPE,
        'P')                                as SPECIALTY_TYPE,
    ...
    left join
        PROVIDER_SPECIALTY_VIEW ps 
            ON p.id = ps.PROVIDER_ID          
    left join
        CORE.SPECIALTY s 
            on s.CODE = ps.SPECIALTY_CODE          
    left join
        core.SPECIALTY gd 
            on gd.CODE = '000'          
    left join
        core.data_owner sdo 
            on sdo.id = ps.DATA_OWNER_ID          
    left join
        FEE_ASSIGNMENT FA 
